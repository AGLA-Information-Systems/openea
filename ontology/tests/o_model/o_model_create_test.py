...
class OModelCreateTestCase(TestCase):
    ...
    def test_o_model_create_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/o_model/create/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
    ...
    def test_o_model_create_page_success(self):
        ...
        response = self.client.get(reverse('o_model_create', kwargs={'repository_id': self.org_1_repo_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_model/create/'+ str(self.org_1_repo_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
