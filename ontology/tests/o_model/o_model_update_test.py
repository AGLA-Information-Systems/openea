...
class OModelUpdateTestCase(TestCase):
    ...
    def test_o_model_update_page_not_authenticated(self):
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_model_update'))
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('o_model_update', kwargs={'pk': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_model/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_model_update', kwargs={'pk': self.org_1_model_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_model/update/' + str(self.org_1_model_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('o_model_update', kwargs={'pk': self.org_1_model_1.id}), data={})
        self.assertRedirects(response, '/user/login/?redirect_to=/o_model/update/' + str(self.org_1_model_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_o_model_update_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        response = self.client.get(reverse('o_model_update', kwargs={'pk': bogus_uuid}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_model_update', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('o_model_update', kwargs={'pk': self.org_1_model_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_model_update', kwargs={'pk': self.org_1_model_1.id}), data={})
    ...
    def test_o_model_update_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_model_update'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_model_update', kwargs={'o_model_id':str(self.org_1_model_1.id)}))

        response = self.client.get(reverse('o_model_update', kwargs={'pk':str(self.org_1_model_1.id)}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'o_model/o_model_update.html')
        response = self.client.post(reverse('o_model_update', kwargs={'pk':str(self.org_1_model_1.id)}), data={'name':'Model 1', 'description': ''})
        self.assertEqual(response.status_code, 200) #TODO : Not 200 in case of error
        response = self.client.post(reverse('o_model_update', kwargs={'pk':str(self.org_1_model_1.id)}), data={'name':'Model 1', 'description': 'Update model', 'repository':self.org_1_repo_1.id})
        self.assertEqual(response.status_code, 302, response.content)

        updated_o_model = OModel.objects.get(id=self.org_1_model_1.id)
        self.assertEqual(updated_o_model.name, 'Model 1')
        self.assertEqual(updated_o_model.description, 'Update model')
        ...
        response = self.client.get(reverse('o_model_update', kwargs={'pk':str(self.org_1_model_1.id)}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_model_update', kwargs={'pk':str(self.org_1_model_1.id)}), data={})
        self.assertEqual(response.status_code, 403)
