...
class OModelDeleteTestCase(TestCase):
    ...
    def test_o_model_delete_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/o_model/delete/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_model_delete', kwargs={'pk': self.org_1_model_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_model/delete/' + str(self.org_1_model_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
