...
from utils.test.helpers import add_object_type_accesspermissions_to_security_group, create_accesspermission, create_instance, create_organisation, create_predicate, create_repository, create_security_group, create_slot, create_user, create_user_profile, create_model, create_concept, create_relation
...
class OSlotListTestCase(TestCase):
    def setUp(self):
        ...
        self.org_1_instance_4 = create_instance(model=self.org_1_model_1, concept=self.org_1_concept_3, name='org_1_instance_4')
        ...
        self.model_1_slot_2 = create_slot(model=self.org_1_model_1, subject=self.org_1_instance_1, predicate=self.org_1_predicate_2, object=self.org_1_instance_3)
        self.model_1_slot_3 = create_slot(model=self.org_1_model_1, subject=self.org_1_instance_1, predicate=self.org_1_predicate_2, object=self.org_1_instance_4)

    def test_o_slot_list_page_not_authenticated(self):
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('o_slot_list', kwargs={'instance_id': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?next=/o_slot/list/'+ str(bogus_uuid), status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('o_slot_list', kwargs={'instance_id': bogus_uuid}), data={})
        self.assertRedirects(response, '/user/login/?next=/o_slot/list/'+ str(bogus_uuid), status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_o_slot_list_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        ...
        response = self.client.get(reverse('o_slot_list', kwargs={'instance_id': self.org_1_instance_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_slot_list', kwargs={'instance_id': self.org_1_instance_1.id}), data={})
        self.assertEqual(response.status_code, 403)

    def test_o_slot_list_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        self.assertContains(response, "org_1_instance_1")
        self.assertContains(response, "org_1_instance_2")
        self.assertContains(response, "org_1_instance_3")
        self.assertContains(response, "org_1_instance_4")

        self.client.logout()

        response = self.client.get(reverse('o_slot_list', kwargs={'instance_id': self.org_1_instance_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_slot/list/'+ str(self.org_1_instance_1.id) , status_code=302, target_status_code=200, fetch_redirect_response=True)
