...
class OSlotDeleteTestCase(TestCase):
    ...
    def test_o_slot_delete_page_not_authenticated(self):
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_slot_delete'))
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('o_slot_delete', kwargs={'pk': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_slot/delete/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_slot_delete', kwargs={'pk': self.model_1_slot_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_slot/delete/' + str(self.model_1_slot_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('o_slot_delete', kwargs={'pk': self.model_1_slot_1.id}), data={})
        self.assertRedirects(response, '/user/login/?redirect_to=/o_slot/delete/' + str(self.model_1_slot_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_o_slot_delete_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        response = self.client.get(reverse('o_slot_delete', kwargs={'pk': bogus_uuid}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_slot_delete', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('o_slot_delete', kwargs={'pk': self.model_1_slot_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_slot_delete', kwargs={'pk': self.model_1_slot_1.id}), data={})
    ...
    def test_o_slot_delete_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_slot_delete'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_slot_delete', kwargs={'o_slot_id':str(self.model_1_slot_1.id)}))

        response = self.client.get(reverse('o_slot_delete', kwargs={'pk':str(self.model_1_slot_1.id)}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'o_slot/o_slot_delete.html')

        response = self.client.post(reverse('o_slot_delete', kwargs={'pk':str(self.model_1_slot_1.id)}), data={})
        self.assertEqual(response.status_code, 302, response.content) #response.context["form"].errors

        with self.assertRaises(OSlot.DoesNotExist):
            deleted_o_slot = OSlot.objects.get(id=self.model_1_slot_1.id)
