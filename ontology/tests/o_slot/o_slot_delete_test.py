...
class OSlotDeleteTestCase(TestCase):
    ...
    def test_o_slot_delete_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/o_slot/delete/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_slot_delete', kwargs={'pk': self.model_1_slot_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_slot/delete/' + str(self.model_1_slot_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
