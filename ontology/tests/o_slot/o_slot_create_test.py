...
from utils.test.helpers import add_object_type_accesspermissions_to_security_group, create_accesspermission, create_instance, create_organisation, create_predicate, create_repository, create_security_group, create_user, create_user_profile, create_model, create_concept, create_relation
...
class OSlotCreateTestCase(TestCase):
    ...
    def test_o_slot_create_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/o_slot/create/'+ str(bogus_uuid) +'/'+ str(bogus_uuid) +'/'+ str(bogus_uuid) +'/0', status_code=302, target_status_code=200, fetch_redirect_response=True)
    ...
    def test_o_slot_create_page_success(self):
        ...
        response = self.client.get(reverse('o_slot_create', kwargs={'instance_id': self.org_1_instance_1.id, 'predicate_id': self.org_1_predicate_1.id, 'concept_id': self.org_1_concept_2.id, 'is_subject': 0}))
        self.assertRedirects(response, '/user/login/?next=/o_slot/create/'+ str(self.org_1_instance_1.id) +'/'+ str(self.org_1_predicate_1.id) +'/'+ str(self.org_1_concept_2.id) +'/0', status_code=302, target_status_code=200, fetch_redirect_response=True)
