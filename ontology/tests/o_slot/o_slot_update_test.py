...
class OSlotUpdateTestCase(TestCase):
    ...
    def test_o_slot_update_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/o_slot/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_slot_update', kwargs={'pk': self.model_1_slot_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_slot/update/' + str(self.model_1_slot_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
