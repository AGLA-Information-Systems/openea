...
from utils.test.helpers import add_object_type_accesspermissions_to_security_group, create_accesspermission, create_concept, create_organisation, create_repository, create_security_group, create_user, create_user_profile, create_model
...
class OConceptListTestCase(TestCase):
    ...
    def test_o_concept_list_page_not_authenticated(self):
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('o_concept_list', kwargs={'model_id': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?next=/o_concept/list/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('o_concept_list', kwargs={'model_id': bogus_uuid}), data={})
        self.assertRedirects(response, '/user/login/?next=/o_concept/list/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_o_concept_list_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        ...
        response = self.client.get(reverse('o_concept_list', kwargs={'model_id': self.org_1_model_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_concept_list', kwargs={'model_id': self.org_1_model_1.id}), data={})
        self.assertEqual(response.status_code, 403)

    def test_o_concept_list_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        response = self.client.get(reverse('o_concept_list', kwargs={'model_id': self.org_1_model_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_concept/list/'+ str(self.org_1_model_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
