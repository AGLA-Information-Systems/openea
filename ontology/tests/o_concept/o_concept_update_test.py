...
from utils.test.helpers import add_object_type_accesspermissions_to_security_group, create_accesspermission, create_model, create_organisation, create_concept, create_repository, create_security_group, create_user, create_user_profile
...
class OConceptUpdateTestCase(TestCase):
    ...
    def test_o_concept_update_page_not_authenticated(self):
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_concept_update'))
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('o_concept_update', kwargs={'pk': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?next=/o_concept/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_concept_update', kwargs={'pk': self.model_1_concept_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_concept/update/' + str(self.model_1_concept_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('o_concept_update', kwargs={'pk': self.model_1_concept_1.id}), data={})
        self.assertRedirects(response, '/user/login/?next=/o_concept/update/' + str(self.model_1_concept_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_o_concept_update_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        response = self.client.get(reverse('o_concept_update', kwargs={'pk': bogus_uuid}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_concept_update', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('o_concept_update', kwargs={'pk': self.model_1_concept_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_concept_update', kwargs={'pk': self.model_1_concept_1.id}), data={})
    ...
    def test_o_concept_update_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_concept_update'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_concept_update', kwargs={'o_concept_id':str(self.model_1_concept_1.id)}))

        response = self.client.get(reverse('o_concept_update', kwargs={'pk':str(self.model_1_concept_1.id)}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'o_concept/o_concept_update.html')
        response = self.client.post(reverse('o_concept_update', kwargs={'pk':str(self.model_1_concept_1.id)}), data={'name':'Concept 1', 'description': ''})
        self.assertEqual(response.status_code, 200) #TODO : Not 200 in case of error
        response = self.client.post(reverse('o_concept_update', kwargs={'pk':str(self.model_1_concept_1.id)}), data={'name':'Concept 1', 'description': 'Update concept', 'quality_status': QUALITY_STATUS_PROPOSED, 'model':self.org_1_model_1.id})
        self.assertEqual(response.status_code, 302, response.content)

        updated_o_concept = OConcept.objects.get(id=self.model_1_concept_1.id)
        self.assertEqual(updated_o_concept.name, 'Concept 1')
        self.assertEqual(updated_o_concept.description, 'Update concept')
        ...
        response = self.client.get(reverse('o_concept_update', kwargs={'pk':str(self.model_1_concept_1.id)}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_concept_update', kwargs={'pk':str(self.model_1_concept_1.id)}), data={})
        self.assertEqual(response.status_code, 403)
