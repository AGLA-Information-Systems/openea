...
from utils.test.helpers import add_object_type_permissions_to_security_group, create_model, create_organisation, create_concept, create_repository, create_security_group, create_user, create_user_profile
...
class OConceptUpdateTestCase(TestCase):
    ...
    def test_o_concept_update_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/o_concept/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_concept_update', kwargs={'pk': self.model_1_concept_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_concept/update/' + str(self.model_1_concept_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
