...
class OConceptDetailTestCase(TestCase):
    ...
    def test_o_concept_detail_page_not_authenticated(self):
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_concept_detail'))
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('o_concept_detail', kwargs={'pk': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?next=/o_concept/detail/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_concept_detail', kwargs={'pk': self.model_1_concept_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_concept/detail/' + str(self.model_1_concept_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('o_concept_detail', kwargs={'pk': self.model_1_concept_1.id}), data={})
        self.assertRedirects(response, '/user/login/?next=/o_concept/detail/' + str(self.model_1_concept_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_o_concept_detail_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        response = self.client.get(reverse('o_concept_detail', kwargs={'pk': bogus_uuid}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_concept_detail', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('o_concept_detail', kwargs={'pk': self.model_1_concept_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_concept_detail', kwargs={'pk': self.model_1_concept_1.id}), data={})
    ...
    def test_o_concept_detail_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_concept_detail'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_concept_detail', kwargs={'o_concept_id':str(self.model_1_concept_1.id)}))

        response = self.client.get(reverse('o_concept_detail', kwargs={'pk':str(self.model_1_concept_1.id)}))
        ...
        response = self.client.get(reverse('o_concept_detail', kwargs={'pk':str(self.model_1_concept_1.id)}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_concept_detail', kwargs={'pk':str(self.model_1_concept_1.id)}), data={})
        self.assertEqual(response.status_code, 403)
