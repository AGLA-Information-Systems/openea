...
class OConceptDetailTestCase(TestCase):
    ...
    def test_o_concept_detail_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/o_concept/detail/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_concept_detail', kwargs={'pk': self.model_1_concept_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_concept/detail/' + str(self.model_1_concept_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
