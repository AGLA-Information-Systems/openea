...
from utils.test.helpers import add_object_type_accesspermissions_to_security_group, create_accesspermission, create_model, create_organisation, create_relation, create_repository, create_security_group, create_user, create_user_profile
...
class ORelationUpdateTestCase(TestCase):
    ...
    def test_o_relation_update_page_not_authenticated(self):
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_relation_update'))
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('o_relation_update', kwargs={'pk': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?next=/o_relation/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_relation_update', kwargs={'pk': self.model_1_relation_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_relation/update/' + str(self.model_1_relation_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('o_relation_update', kwargs={'pk': self.model_1_relation_1.id}), data={})
        self.assertRedirects(response, '/user/login/?next=/o_relation/update/' + str(self.model_1_relation_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_o_relation_update_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        response = self.client.get(reverse('o_relation_update', kwargs={'pk': bogus_uuid}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_relation_update', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('o_relation_update', kwargs={'pk': self.model_1_relation_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_relation_update', kwargs={'pk': self.model_1_relation_1.id}), data={})
    ...
    def test_o_relation_update_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_relation_update'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('o_relation_update', kwargs={'o_relation_id':str(self.model_1_relation_1.id)}))

        response = self.client.get(reverse('o_relation_update', kwargs={'pk':str(self.model_1_relation_1.id)}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'o_relation/o_relation_update.html')
        response = self.client.post(reverse('o_relation_update', kwargs={'pk':str(self.model_1_relation_1.id)}), data={'name':'Relation 1', 'description': ''})
        self.assertEqual(response.status_code, 200) #TODO : Not 200 in case of error
        response = self.client.post(reverse('o_relation_update', kwargs={'pk':str(self.model_1_relation_1.id)}), data={'name':'Relation 1', 'type': ORelation.PROPERTY,'description': 'Update relation', 'quality_status': QUALITY_STATUS_PROPOSED, 'model':self.org_1_model_1.id})
        self.assertEqual(response.status_code, 302, response.content)

        updated_o_relation = ORelation.objects.get(id=self.model_1_relation_1.id)
        self.assertEqual(updated_o_relation.name, 'Relation 1')
        self.assertEqual(updated_o_relation.description, 'Update relation')
        ...
        response = self.client.get(reverse('o_relation_update', kwargs={'pk':str(self.model_1_relation_1.id)}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('o_relation_update', kwargs={'pk':str(self.model_1_relation_1.id)}), data={})
        self.assertEqual(response.status_code, 403)
