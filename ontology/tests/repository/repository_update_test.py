...
class RepositoryUpdateTestCase(TestCase):
    ...
    def test_repository_update_page_not_authenticated(self):
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('repository_update'))
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('repository_update', kwargs={'pk': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?redirect_to=/repository/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('repository_update', kwargs={'pk': self.org_1_repo_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/repository/update/' + str(self.org_1_repo_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('repository_update', kwargs={'pk': self.org_1_repo_1.id}), data={})
        self.assertRedirects(response, '/user/login/?redirect_to=/repository/update/' + str(self.org_1_repo_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_repository_update_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        response = self.client.get(reverse('repository_update', kwargs={'pk': bogus_uuid}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('repository_update', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('repository_update', kwargs={'pk': self.org_1_repo_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('repository_update', kwargs={'pk': self.org_1_repo_1.id}), data={})
    ...
    def test_repository_update_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('repository_update'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('repository_update', kwargs={'repository_id':str(self.org_1_repo_1.id)}))

        response = self.client.get(reverse('repository_update', kwargs={'pk':str(self.org_1_repo_1.id)}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'repository/repository_update.html')
        response = self.client.post(reverse('repository_update', kwargs={'pk':str(self.org_1_repo_1.id)}), data={'name':'Repo 1', 'description': ''})
        self.assertEqual(response.status_code, 302)
        response = self.client.post(reverse('repository_update', kwargs={'pk':str(self.org_1_repo_1.id)}), data={'name':'Repo 1', 'description': 'Update repo', 'organisation':self.org_1.id})
        self.assertEqual(response.status_code, 302)

        updated_repository = Repository.objects.get(id=self.org_1_repo_1.id)
        self.assertEqual(updated_repository.name, 'Repo 1')
        self.assertEqual(updated_repository.description, 'Update repo')
        ...
        response = self.client.get(reverse('repository_update', kwargs={'pk':str(self.org_1_repo_1.id)}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('repository_update', kwargs={'pk':str(self.org_1_repo_1.id)}), data={})
        self.assertEqual(response.status_code, 403)
