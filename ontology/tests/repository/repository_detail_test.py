...
class RepositoryDetailTestCase(TestCase):
    ...
    def test_repository_detail_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/repository/detail/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('repository_detail', kwargs={'pk': self.org_1_repo_1.id}))
        self.assertRedirects(response, '/user/login/?next=/repository/detail/' + str(self.org_1_repo_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
