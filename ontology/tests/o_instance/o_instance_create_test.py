...
from utils.test.helpers import add_object_type_accesspermissions_to_security_group, create_accesspermission, create_model, create_organisation, create_repository, create_security_group, create_user, create_user_profile, create_concept
...
class OInstanceCreateTestCase(TestCase):
    ...
    def test_o_instance_create_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/o_instance/create/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
    ...
    def test_o_instance_create_page_success(self):
        ...
        response = self.client.get(reverse('o_instance_create', kwargs={'concept_id': self.org_1_concept_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_instance/create/'+ str(self.org_1_concept_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
