...
class OInstanceDeleteTestCase(TestCase):
    ...
    def test_o_instance_delete_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/o_instance/delete/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_instance_delete', kwargs={'pk': self.model_1_instance_1.id}))
        self.assertRedirects(response, '/user/login/?next=/o_instance/delete/' + str(self.model_1_instance_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
