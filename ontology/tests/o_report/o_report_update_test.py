...
from utils.test.helpers import add_object_type_permissions_to_security_group, create_model, create_organisation, create_report, create_repository, create_security_group, create_user, create_user_profile
...
class OReportUpdateTestCase(TestCase):
    ...
    def test_o_report_update_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/o_report/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_report_update', kwargs={'pk': self.model_1_report_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_report/update/' + str(self.model_1_report_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
