...
class OReportDeleteTestCase(TestCase):
    ...
    def test_o_report_delete_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/o_report/delete/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('o_report_delete', kwargs={'pk': self.model_1_report_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_report/delete/' + str(self.model_1_report_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
