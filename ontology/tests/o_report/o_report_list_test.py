...
from utils.test.helpers import add_object_type_permissions_to_security_group, create_report, create_organisation, create_repository, create_security_group, create_user, create_user_profile, create_model
...
class OReportListTestCase(TestCase):
    ...
    def test_o_report_list_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/o_report/list/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
    ...
    def test_o_report_list_page_success(self):
        ...
        response = self.client.get(reverse('o_report_list', kwargs={'model_id': self.org_1_model_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/o_report/list/'+ str(self.org_1_model_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
