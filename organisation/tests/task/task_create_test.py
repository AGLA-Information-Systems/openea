...
class TaskCreateTestCase(TestCase):
    ...
    def test_task_create_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/task/create/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
    ...
    def test_task_create_page_success(self):
        ...
        response = self.client.get(reverse('task_create', kwargs={'organisation_id': self.org_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/task/create/'+ str(self.org_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
