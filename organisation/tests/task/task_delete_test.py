...
class TaskDeleteTestCase(TestCase):
    ...
    def test_task_delete_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/task/delete/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('task_delete', kwargs={'pk': self.org_1_task_1.id}))
        self.assertRedirects(response, '/user/login/?next=/task/delete/' + str(self.org_1_task_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
