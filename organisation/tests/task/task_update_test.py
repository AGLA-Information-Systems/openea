...
class TaskUpdateTestCase(TestCase):
    ...
    def test_task_update_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?next=/task/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('task_update', kwargs={'pk': self.org_1_task_1.id}))
        self.assertRedirects(response, '/user/login/?next=/task/update/' + str(self.org_1_task_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
