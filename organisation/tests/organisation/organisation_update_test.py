...
class OrganisationUpdateTestCase(TestCase):
    ...
    def test_organisation_update_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/organisation/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('organisation_update', kwargs={'pk': self.org_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/organisation/update/' + str(self.org_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
