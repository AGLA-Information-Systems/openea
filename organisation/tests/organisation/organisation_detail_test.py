...
class OrganisationDetailTestCase(TestCase):
    def setUp(self):
        ...
        self.perm = create_permission(security_group=self.org_1_security_group_1, action='VIEW', object_type=self.object_type)
    ...
    def test_organisation_detail_page_not_authenticated(self):
        ...
        self.assertRedirects(response, '/user/login/?redirect_to=/organisation/detail/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('organisation_detail', kwargs={'pk': self.org_1.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/organisation/detail/' + str(self.org_1.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
