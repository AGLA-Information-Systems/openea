...
class OrganisationDeleteTestCase(TestCase):
    ...
    def test_organisation_delete_page_authenticated_not_allowed(self):
        ...
        response = self.client.post(reverse('organisation_delete', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('organisation_delete', kwargs={'pk': self.org_1.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('organisation_delete', kwargs={'pk': self.org_1.id}), data={})
    ...
    def test_organisation_delete_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('organisation_delete'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('organisation_delete', kwargs={'organisation_id':str(self.org_1.id)}))

        response = self.client.get(reverse('organisation_delete', kwargs={'pk':str(self.org_1.id)}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'organisation/organisation_delete.html')

        response = self.client.post(reverse('organisation_delete', kwargs={'pk':str(self.org_1.id)}), data={})
        ...
        response = self.client.get(reverse('organisation_delete', kwargs={'pk':str(self.org_2.id)}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('organisation_delete', kwargs={'pk':str(self.org_2.id)}), data={})
        self.assertEqual(response.status_code, 403)
