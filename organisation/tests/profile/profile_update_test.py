...
class ProfileUpdateTestCase(TestCase):
    def setUp(self):
        ...
        self.perm = create_permission(security_group=self.org_1_security_group_1, action='UPDATE', object_type=self.object_type)
    ...
    def test_profile_update_page_not_authenticated(self):
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('profile_update'))
        bogus_uuid = uuid.uuid4()
        response = self.client.get(reverse('profile_update', kwargs={'pk': bogus_uuid}))
        self.assertRedirects(response, '/user/login/?redirect_to=/profile/update/'+ str(bogus_uuid) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.get(reverse('profile_update', kwargs={'pk': self.org_1_user_1_profile.id}))
        self.assertRedirects(response, '/user/login/?redirect_to=/profile/update/' + str(self.org_1_user_1_profile.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        response = self.client.post(reverse('profile_update', kwargs={'pk': self.org_1_user_1_profile.id}), data={})
        self.assertRedirects(response, '/user/login/?redirect_to=/profile/update/' + str(self.org_1_user_1_profile.id) +'/', status_code=302, target_status_code=200, fetch_redirect_response=True)
        
    def test_profile_update_page_authenticated_not_allowed(self):
        bogus_uuid = uuid.uuid4()
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        response = self.client.get(reverse('profile_update', kwargs={'pk': bogus_uuid}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('profile_update', kwargs={'pk': bogus_uuid}), data={})
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('profile_update', kwargs={'pk': self.org_1_user_1_profile.id}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('profile_update', kwargs={'pk': self.org_1_user_1_profile.id}), data={})
    ...
    def test_profile_update_page_success(self):
        logged_in = self.client.login(username='org_1_user_1', password='12345')
        self.assertTrue(logged_in)
        ...
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('profile_update'))
        with self.assertRaises(NoReverseMatch):
            response = self.client.get(reverse('profile_update', kwargs={'profile_id':str(self.org_1_user_1_profile.id)}))

        response = self.client.get(reverse('profile_update', kwargs={'pk':str(self.org_1_user_1_profile.id)}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'profile/profile_update.html')
        response = self.client.post(reverse('profile_update', kwargs={'pk':str(self.org_1_user_1_profile.id)}), data={'name':'Profile 1', 'description': ''})
        self.assertEqual(response.status_code, 200) #TODO : Not 200 in case of error
        response = self.client.post(reverse('profile_update', kwargs={'pk':str(self.org_1_user_1_profile.id)}), data={'role':'Admin', 'description': 'Update profile', 'organisation':self.org_1.id})
        self.assertEqual(response.status_code, 302)

        updated_profile = Profile.objects.get(id=self.org_1_user_1_profile.id)
        self.assertEqual(updated_profile.name, 'Admin')
        self.assertEqual(updated_profile.description, 'Update profile')
        ...
        response = self.client.get(reverse('profile_update', kwargs={'pk':str(self.org_1_user_1_profile.id)}))
        self.assertEqual(response.status_code, 403)
        response = self.client.post(reverse('profile_update', kwargs={'pk':str(self.org_1_user_1_profile.id)}), data={})
        self.assertEqual(response.status_code, 403)
